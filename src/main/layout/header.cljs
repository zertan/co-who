...
(defn header-comp [logo]
  (dom/header {}
              (dom/nav  {:class "border-gray-200 text-gray-900
                bg-[#f3f4f6] dark:bg-[#101014]
                dark:border-gray-800 dark:text-gray-400"}
                (dom/div  {:class "flex flex-wrap justify-between items-center mx-auto max-w-screen-xl"}
                  (dom/a  {:href "https://flowbite.com", :class "flex items-center"}
                         (dom/img  {:src "https://flowbite.com/docs/images/logo.svg"
                                       :class "mr-3 h-6 sm:h-9"
                                       :alt "Flowbite Logo"} "asd")
                    (dom/span  {:class "self-center text-xl font-semibold whitespace-nowrap dark:text-white"}
                      "Flowbite"))
                  (dom/div  {:class "flex items-center lg:order-2"}
                    (search-comp)
                    (dom/a 
                        {:href "#",
                         :class
                  ...
                  (dom/div  {:class "hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1"}
                    (dom/ul {:class "flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0"}
                            (doall
                             (for [t ["Home" "Company" "Marketplace" "Features" "Team" "Company"]]
                               (list-comp t)))))))))
